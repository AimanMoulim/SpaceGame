...
export function PixelRunnerGame({ onSessionData }: any) {
  ...
  useEffect(() => {
    ...
    gameStateRef.current.playerY = (containerHeight / 600) * 450
  ...
  return (
    <div
      ref={gameRef}
      className="w-full h-screen bg-gradient-to-b from-green-400 to-green-600 flex flex-col items-center justify-center p-2 sm:p-4 overflow-hidden"
    >
      <h1 className="text-3xl sm:text-4xl font-bold text-white mb-3 sm:mb-4 drop-shadow-lg">Pixel Runner</h1>

      <div className="w-full max-w-full sm:max-w-md border-4 border-white rounded-lg overflow-hidden mb-3 sm:mb-4 drop-shadow-lg flex-shrink-0">
      ...
      <div className="text-white text-center text-xs sm:text-base flex-shrink-0">
        <p className="mb-2 font-bold">Space or Up Arrow to jump</p>

        {gameOver && (
          <div className="bg-black/70 backdrop-blur-sm rounded-lg p-4 sm:p-8 mt-2 sm:mt-4 max-w-sm">
            <h2 className="text-2xl sm:text-3xl font-bold text-yellow-400 mb-3 sm:mb-4">Game Over!</h2>
            ...
            <button
              onClick={resetGame}
              className="px-6 py-2 sm:py-3 bg-gradient-to-r from-yellow-400 to-orange-400 text-black font-bold rounded-lg hover:from-yellow-500 hover:to-orange-500 transition-all text-sm sm:text-base"
            >
              Play Again
            </button>
          </div>
